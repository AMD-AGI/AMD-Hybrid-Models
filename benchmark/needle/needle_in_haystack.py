...
class LLMNeedleHaystackTester:
    ...
    def __init__(self,
        ...
        if(self.model_provider not in ["OpenAI", "Anthropic"]):
            ...
            if "mla" in test_name.lower() or "mla" in self.model_provider.lower():
                print("MLA model_name 2:", model_name)
                sys.path.append('./')
                from mla.hybrid_wrapper import MLATransformerHybridModelWrapper
                self.model_to_test = MLATransformerHybridModelWrapper.from_pretrained(model_name, torch_dtype=torch.bfloat16).model.cuda()
                # self.model_to_test.config.use_cache = False
                # self.model_to_test.generation_config.use_cache = False
                print(self.model_to_test)
                print("============")
            elif test_name == "llama-2-7b-hf" or test_name == "llama-2-7b-hf-slimpajama-pi-32k" or test_name == "llama-2-7b-hf-slimpajama-longlora-32k":
            ...
            elif test_name == "llama-2-7b-hf-yarn" or test_name == "llama-2-7b-hf-slimpajama-yarn-32k":
                print(f'testing {test_name}')
                ...
                print(config.rope_scaling)
                self.model_to_test = model_cls.from_pretrained(model_name,
                                                        use_flash_attention_2="flash_attention_2", 
                                                        torch_dtype=torch.bfloat16,
                                                        ).eval()
                ...
                self.model_to_test = tp.tensor_parallel(self.model_to_test, sharded=True)
            elif test_name == "llama-2-7b-hf-slimpajama-ntk-64k" or test_name == "llama-2-7b-hf-slimpajama-ntk-64k-2B":
                print(f'testing {test_name}')
                config = transformers.AutoConfig.from_pretrained(model_name)
                print(config.rope_scaling)
                ...
                self.model_to_test = tp.tensor_parallel(self.model_to_test, sharded=True)
            elif test_name == "llama-2-7b-hf-ntk-frozen":
                print(f'testing {test_name}')
                config = transformers.AutoConfig.from_pretrained(model_name)
                scaling_factor = 2.0
                ...
                self.model_to_test = tp.tensor_parallel(self.model_to_test, sharded=True)
            elif test_name == "llama-2-7b-hf-landmark" or test_name == "llama-2-7b-hf-slimpajama-landmark" :
                ...
                self.model_to_test.set_mem_id(mem_id)
                self.model_to_test.enable_flash()
                self.model_to_test = tp.tensor_parallel(self.model_to_test, sharded=True)

            elif test_name == "llama-2-7b-hf-lminfinite":
                ...
                self.model_to_test = convert_llama_model(self.model_to_test, 4096, 10)
    ...
    def evaluate_and_log(self, context_length, depth_percent):
        ...
        if(self.model_provider in ["OpenAI", "Anthropic"]):
            ...
            if self.model_version == "llama-2-7b-hf-landmark" or self.model_version == "llama-2-7b-hf-slimpajama-landmark":
                kwargs = {}
                ...
                with torch.no_grad():
                    output_ids = self.model_to_test.generate(input_ids, max_new_tokens=50, **kwargs)
        ...
        if self.save_contexts:
            results['file_name'] : context_file_location # type: ignore
    ...
    def result_exists(self, context_length, depth_percent):
        ...
        results_dir = os.path.join('./results/',self.model_version)
    ...
    def encode_text_to_tokens(self, text):
        if self.model_provider in ["OpenAI", "LLaMA", "Mistral", "GLM", "MLA"]:
    ...
    def insert_needle(self, context, depth_percent, context_length):
        ...
        if depth_percent == 100:
            ...
            if(self.model_provider in ["LLaMA", "LongLLaMA", "MLA"]): period_tokens = [29889, 869]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--test_name', type=str, default=None, help='name of test')
