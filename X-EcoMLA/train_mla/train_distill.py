...
def main():
    ...
    if training_args.prev_checkpoint_path is not None:
        ...
        prev_checkpoint_layers, is_mla_layer = construct_layer_dict(prev_checkpoint, config.num_hidden_layers)
        mla_layers = training_args.mla_layers
        for (layer_id, layer_checkpoint) in prev_checkpoint_layers.items():
            if is_mla_layer[layer_id]:
                # override weights of mla that layer
                model.model.model.layers[layer_id].load_state_dict(layer_checkpoint)
            
            elif layer_id in mla_layers:
                # previous transformer layers, but now mla layers, apply kvq transfomer
                ...
                model.model.model.layers[layer_id].mla.out_proj.load_state_dict(self_attn_o_proj_state_dict)

                # TODO: the checkpoint reloading need to updated
                model.model.model.layers[layer_id].mla.init_with_kqv(
                                                                     self_attn_q_proj_state_dict['weight'].data,
                                                                     self_attn_k_proj_state_dict['weight'].data,
                                                                     self_attn_v_proj_state_dict['weight'].data,
                                                                    )
                q_matrix = self_attn_q_proj_state_dict['weight'].data                   
                q_rank = model.model.model.layers[layer_id].mla.re_init_q(q_matrix, model_args.torch_dtype)

                k_matrix = self_attn_k_proj_state_dict['weight'].data
                v_matrix = self_attn_v_proj_state_dict['weight'].data
                kv_rank = model.model.model.layers[layer_id].mla.re_init_kv(k_matrix, v_matrix, mla_config.head_dim, model_args.torch_dtype)
                print(f"layerid: {layer_id}; q_rank: {q_rank}; kv_rank: {kv_rank}; total_rank: {q_rank+kv_rank}")
                print("init here.")
            else:
