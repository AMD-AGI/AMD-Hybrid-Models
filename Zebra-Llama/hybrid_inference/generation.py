...
def decode(
    ...
    if cg:
        if not cg_piecewise:
            ...
            inf_cache = model.allocate_inference_cache(batch_size, max_length, dtype)
            ...
            model.capture_graph(inference_params)
    ...
    prof = torch.profiler.profile(
        schedule=torch.profiler.schedule(wait=1, warmup=50, active=2),
        activities=[torch.profiler.ProfilerActivity.CPU, torch.profiler.ProfilerActivity.CUDA],
        on_trace_ready=torch.profiler.tensorboard_trace_handler('./log/'),
        record_shapes=False,
        profile_memory=False,
        with_stack=True
    ) if profile is True else contextlib.nullcontext()
    ...
    with prof as p:
        ...
        while not should_stop(sequences[-1], inference_params):
            ...
            if inference_params.seqlen_offset == 0 and random_context: 
                logits = torch.randn((sequences[-1].shape[0], 128256), dtype=torch.float16).to(sequences[-1].device)
            else: 
                logits = get_logits(sequences[-1], inference_params)
    ...
    if enable_timing:
        ...
        tokens_per_second = total_tokens / time * 1e3
        print(f"Correct decoding throughput!!!: {(tokens_per_second):.2f} tokens/s")
